...
export default function Home() {
    ...
    try {
      ...
      if (result.detections.length === 0) {
        toast({
          ...
          description: "No license plates were found in the image.",
        });
      } else {
        toast({
          title: "Detection Complete",
          description: `Found ${result.detections.length} license plate${result.detections.length > 1 ? "s" : ""}.`,
        });
      }
    } catch (err) {
  ...
  const getConfidenceColor = (conf: number): string => {
    if (conf >= 0.8) return "bg-chart-2"; // Green
    if (conf >= 0.5) return "bg-chart-3"; // Yellow
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {/* TODO: Future blur logic will be implemented here
                When enabled, the backend will return an additional blurred image
                with license plate regions obscured for privacy protection */}
          </CardContent>
        </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <img
                    ...
                    alt="Image with detected license plates"
                    className="w-full h-auto rounded-lg object-contain max-h-80"
              ...
              <CardHeader>
                <CardTitle className="flex items-center justify-between gap-2 flex-wrap">
                  ...
                  <span className="text-sm font-normal text-muted-foreground">
                    {detectionResult.detections.length} plate{detectionResult.detections.length !== 1 ? "s" : ""} detected
                  </span>
                </CardTitle>
              </CardHeader>
              <CardContent>
...
function DetectionCard({ detection, index, getConfidenceColor }: DetectionCardProps) {
