...
# Allow frontend calls (adjust origins in production)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
if not os.path.exists(WEIGHTS_PATH):
    # Don't crash import-time in some deployments; raise at runtime when called
    model = None
else:
    model = YOLO(WEIGHTS_PATH)

def _load_image_from_upload(file_bytes: bytes) -> np.ndarray:
    """Decode uploaded bytes to BGR image (OpenCV)."""
...
@app.get("/health")
def health():
    return {"status": "ok", "weights": WEIGHTS_PATH, "model_loaded": model is not None}
